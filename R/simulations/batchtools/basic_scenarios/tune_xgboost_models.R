...
# Linear smooth
data_linear_smooth = create_sim_data(n = 1000, type = "linear_smooth")$data
...
prediction_linear_smooth = learner_linear_smooth$predict(task_linear_smooth)
...
summary(lm(y ~ x1 + x2 + x5 + x2:x3 + x4:x5, data = data_linear_smooth))

x_linear_smooth = data_linear_smooth[which(names(data_linear_smooth) != "y")]

predictor_linear_smooth = Predictor$new(learner_linear_smooth, data = x_linear_smooth, y = data_linear_smooth$y)
...
summary(gam(y ~ x1 + x2 + x2:x3 + ti(x1,x2), data = data_linear_abrupt))
...
summary(gam(y ~ x2 + x4 + x6 + x8 + x2:x1 + x3:x2 + x5:x2:x6 + x2:x7 + x1:x3 + x8:x10 + x7:x9, data = data_linear_mixed))
...
prediction_linear_mixed = at$predict(task_linear_mixed)
...
predictor_linear_mixed = Predictor$new(at, data = x_linear_mixed, y = data_linear_mixed$y)

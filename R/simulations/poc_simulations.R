...
data_linear = create_sim_data(job = NULL, n = n, type = "basic_linear_smooth")$data
X_linear = data_linear[which(names(data_linear)!="y")]
y_linear = data_linear$y 

tree_linear = compute_tree_slim(y_linear, X_linear, n.split = 2, penalization = NULL, fit.bsplines = TRUE, impr.par = 0.1, min.split = n/100, pruning = "forward",
                                split.method = "slim")
res_linear = extract_split_criteria(tree_linear)

tree_linear_guide = compute_tree_slim(y_linear, X_linear, n.split = 2, penalization = NULL, fit.bsplines = TRUE, impr.par = 0.1, min.split = n/100, pruning = "forward",
                                split.method = "guide")
res_linear_guide = extract_split_criteria(tree_linear)
...
data_categorical = create_sim_data(job = NULL, n = n, type = "categorical_linear")$data
...
tree_categorical_guide = compute_tree_slim(y_categorical, X_categorical, n.split = 3, 
                                           penalization = NULL, degree.poly = 1, 
                                           impr.par = 0.01, min.split = n/100, pruning = "forward",
                                           split.method = "guide")
res_categorical_guide = extract_split_criteria(tree_categorical)
...
data_linear_mixed = create_sim_data_slim(n = n, type = "linear_mixed")$data
...
data_mixed_large = create_sim_data_slim(job = NULL, n = n, type = "mixed_large")
...
n = 1000
data_nonlinear = create_sim_data_slim(n = n, type = "nonlinear")$data
...
start = Sys.time()
tree_nonlinear_gam = compute_tree_slim(y_nonlinear, X_nonlinear, n.split = 4, fit.gam = TRUE, impr.par = 0.1, min.split = 100, pruning = "forward")
print(Sys.time() - start)
...
# slim 
n = 10000
data_slim = create_sim_data_slim(nob = NULL, n = n, type = "slim2")$data
X_slim = data_slim[which(names(data_slim)!="y")]
y_slim = data_slim$y 

start = Sys.time()
tree_slim_splines = compute_tree_slim(y_slim, X_slim, n.split = 3, fit.bsplines = TRUE, impr.par = 0.1, min.split = max(n/100,20), pruning = "forward")
print(Sys.time() - start)
...
start = Sys.time()
tree_slim_lasso = compute_tree_slim(y_slim, X_slim, n.split = 3, fit.bsplines = FALSE, penalization = "L1", degree.poly = 3, impr.par = 0.1, min.split = max(n/100,20), pruning = "forward")
print(Sys.time() - start)

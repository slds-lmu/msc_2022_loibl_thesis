...
\subsubsection{SLIM}
SLIM by \citet{Hu.2020} is explicitly designed to create surrogate MBTs that contain main effect models in the leafnodes and are split by interactions. 
...
As prepruning criterion, MOB uses the Bonferroni-adjusted p-value of the M-flucutation test. That means a split is only performed if the instability is significant at a given significance level $alpha$.

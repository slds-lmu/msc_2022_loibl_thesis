...
In contrast to the definition in \citet{Zeileis.2008}, $\textbf{z} = \textbf{x}$ was set in (\ref{align:W_j}) as no distinction is made between regressor and partitiong variables.
...
CTree follows a very similar approach to MOB and also tries to detect parameter instability by analysing the dependecy between potential splitting variables and a transformation $h()$ of the response $y$.

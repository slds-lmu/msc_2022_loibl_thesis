...
In figure \ref{fig:lm_1000_standalone_r2_test} it can be seen that SLIM and GUIDE achieve better test accuracy than MOB and CTree with the same number of leafnodes. If we also look at Figure \ref{fig:lm_1000_standalone_share_x3_x4.png}, in which the proportion of observations split by $x_1$ or $x_2$ is plotted against the number of leafnodes, it becomes clear that SLIM and GUIDE use the abrupt changepoints defined through the features $x_3$ and $x_4$ for the split more often than the other two methods, which could explain the better performance of SLIM and GUIDE.
